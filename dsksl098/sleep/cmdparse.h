 ...
 *
 * DESCRIPTIVE NAME = Basic data types for Command Parser
 *
 *
 *
 *
 ...
 *
 *	       CMDPDSKT.C - Command Table for Diskette ADD
 *	       CMDPDISK.C - Command Table for ST-506 DASD ADDs
 *	       CMDPSCSI.C - Command Table for SCSI ADD
 *
 *
 * FUNCTIONS  :
 *
 ...
 *
 * NOTES
 * 940215/VJ: added TYPE_DEV_ID to support GDT addressing
 *	      (both CACHE and SCSI services)
 *
 *
 * STRUCTURES
 *
...
**
**   varies    TYPE_SCSI_ID   d,... and (d,d),... scsi target id
**				     where d = 0-7
**
...
**			then divide by 2.
**
**   varies    TYPE_DEV_ID    c,... (c,d),... and (c,d,d),... device id
**				     where d = 0-7
**				     and   c = 0-9 (cache device or scsi channel)
**
**			In the output buffer the TYPE_DEV token
**			contains varying number of 3 byte field values.
**			The first contains c, second the SCSI Target Id and the
**			third contains the LUN id (Target Id and LUN will be
**			set to -1 if not supplied
**
**			To determine the number of 3 byte fields, in
**			the token string -- subtract TOK_MIN_LENGTH
**			from the contents of the token length field and
**			then divide by 3.
**
...
typedef struct _opt {
    UCHAR	id;			/* user defined */
    UCHAR	reserved;		/* (padding) */
    PSZ		string;			/* user defined */
    BYTE	type;			/* user selected from list below */
    SHORT 	state[];		/* user selected */
} OPT, FAR *POPT;
...
#define TOK_ID_END	0xFF			/* denotes end of token string */
...
#define TYPE_DEV_ID	 11    /* d, (d,d) or (d,d,d)		    varies */
#define TYPE_DDDD	12		/* dddd digit			2 */
#define TYPE_ULIST	13		/* (d,d),(d,d,d),...	   varies */
...
CC FAR Command_Parser(PSZ,POPTIONTABLE,PBYTE,USHORT); /* CMDPARSE.C	 */
